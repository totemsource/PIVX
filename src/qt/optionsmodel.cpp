...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::TOTENX);
    ...
    if (!settings.contains("nAnonymizeTotemAmount"))
        settings.setValue("nAnonymizeTotemAmount", 1000);

    nAnonymizeTotemAmount = settings.value("nAnonymizeTotemAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeTotemAmount"))
        SoftSetArg("-anonymizetotemamount", settings.value("nAnonymizeTotemAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in totem.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeTotemAmount:
            return QVariant(nAnonymizeTotemAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeTotemAmount:
            nAnonymizeTotemAmount = value.toInt();
            settings.setValue("nAnonymizeTotemAmount", nAnonymizeTotemAmount);
            emit anonymizeTotemAmountChanged(nAnonymizeTotemAmount);
            break;
        case CoinControlFeatures:
