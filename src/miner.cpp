...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("TOTEMMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("TOTEMMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("TOTEMMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("totem-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running TOTEMMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
